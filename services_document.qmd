...
Results from executing equation 3 to 6 is shown in @tbl-2. There are four column, with the first two use log manufactures exports as the left hand-side variable while the latter two uses log manufacturing GDP. `ARDL(1,0,0)` is used for the first column and `ARDL(1,1,1)` is used for the second column of each, as discussed in the previous section.
